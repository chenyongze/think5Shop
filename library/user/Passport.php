...
class Passport
{
    ...
    public function getUserData($param = null)
    {
        ...
        if (isset($param['uId']) && $param['uId']) {
            ...
            //用户名查询 用于登录查询密码  判断重名
			$column = $this->userType == 'admin' ? ', action_list' : '';
            $userData = Db::table($this->userType)
                ->field($param['localType'] . ', id, password, createtime' . $column)
    ...
    {
        $columns = [
            'members' => ['id', 'local', 'nick_name', 'email', 'mobile'],
            'admin' => ['id', 'local', 'nick_name', 'role_id', 'is_super'],
        ];
